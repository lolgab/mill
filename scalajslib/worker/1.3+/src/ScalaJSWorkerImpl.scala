...
import org.scalajs.linker.{PathIRContainer, PathIRFile, PathOutputDirectory, PathOutputFile, StandardImpl}
import org.scalajs.linker.interface.{ModuleKind => ScalaJSModuleKind, ModuleSplitStyle => ScalaJSModuleSplitStyle, _}
...
class ScalaJSWorkerImpl extends mill.scalajslib.api.ScalaJSWorkerApi {
  private case class LinkerInput(fullOpt: Boolean, moduleKind: ModuleKind, moduleSplitStyle: ModuleSplitStyle, useECMAScript2015: Boolean)
  ...
  def link(sources: Array[File],
    ...
    val linker = ScalaJSLinker.reuseOrCreate(LinkerInput(fullOpt, moduleKind, moduleSplitStyle, useECMAScript2015))
    ...
    val linkerOutput = PathOutputDirectory(jsFile)
