...
object SemanticdbProcessor {
  ...
  case class SourceFile(path: os.Path, code: String)

  def postProcess(
      module: SourceFile,
      wd: Option[os.Path],
      adjust: (Int, Int) => Option[(Int, Int)],
      orig: os.Path,
      dest: os.Path
  ): Unit = {
    val mapRange = {
      (range: scala.meta.internal.semanticdb.Range) =>
        for {
          (startLine, startChar) <- adjust(range.startLine, range.startCharacter)
          (endLine, endChar) <- adjust(range.endLine, range.endCharacter)
        } yield range
